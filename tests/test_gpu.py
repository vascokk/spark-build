...
def _submit_gpu_app(num_executors, executor_gpus, gpus_max, app_name=None):
    ...
    driver_task_id = utils.submit_job(
        app_url=utils.scala_test_jar_url(),
        app_args=app_args,
        args=args)
...
def test_executor_gpus_exceeds_available_gpus():
    ...
    try:
        ...
        utils.kill_driver(driver_task_id, utils.SPARK_APP_NAME)
        return
...
def test_gpus_max():
    ...
    # TODO: Actually, we expect total gpus == gpus.max because gpus are allocated greedily.
    # Uncomment the following line when this is fixed in the Spark PR.
    #assert total_gpus == gpus_max
